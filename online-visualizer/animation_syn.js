...
async function main() {
  ...
  const res2 = await fetch("/data_syn.json");
  const data_syn = await res2.json();
  displayData(data_syn, data);
}
/**
 * Finds the minimum and maximum values in data.var_plot.
...
/**
 * Rounds the minimum and maximum values to a certain number of significant figures.
 * @param {number} varMin - The minimum value to be rounded.
 * @param {number} varMax - The maximum value to be rounded.
 * @returns {Object} An object containing the rounded minimum and maximum values.
 */
...
/**
 * Displays data on a three.js scene and handles mouse click events.
 * @param {Object} data_syn - The data to be displayed.
 * @param {Object} data - Additional data needed for display.
 */
function displayData(data_syn, data) {
  const scene = new THREE.Scene();

  let { varMin, varMax } = findMinMax(data);
  ...
  /**
   * Gets the color corresponding to a normalized value.
   * @param {number} varNormalized - The normalized value.
   * @returns {Object} The color corresponding to the normalized value.
   */
   ...
   * @returns {string} The generated color in rgb format.
   */
  ...
  /**
   * Adds a glow effect to a section.
   * @param {number} sectionIndex - The index of the section to add the glow effect to.
  ...
  function addGlowToSection(sectionIndex, color) {
    for (let i = 0; i < data.positions.length - 1; i++) {
      if (data.positions[i][5] === sectionIndex) {
  ...
  for (let i = 0; i < 256; i++) {
    ...
    colorStripe.style.background = `rgb(${color.r * 255}, ${color.g * 255}, ${
      color.b * 255
    })`;
  ...
  for (let i = 0; i < data.positions.length - 1; i++) {
    ...
    const section_index = data.positions[i][5];
    console.log(section_index);
    const next_section_index = data.positions[i + 1][5];
    const previous_section_index = data.positions[Math.max(0, i - 1)][5];
  ...
  function animate() {
    ...
    frame =
      (frame + time_factor + data.var_plot[0].length) % data.var_plot[0].length;
