...
#define trap0 trap0_na3
 ...
 extern double alpv( _threadargsprotocomma_ double , double );
...
#define hmin hmin_na3
 double hmin = 0.5;
#define hscale hscale_na3
 double hscale = 1;
#define mmin mmin_na3
 double mmin = 0.02;
#define mscale mscale_na3
 double mscale = 1;
#define qq qq_na3
 double qq = 10;
#define q10 q10_na3
...
#define qinf qinf_na3
 double qinf = 4;
#define smin smin_na3
 double smin = 10;
#define sscale sscale_na3
 double sscale = 1;
#define tq tq_na3
 double tq = -55;
...
#define zetas zetas_na3
 ...
 "vvh_na3", &vvh_na3,
 "vvs_na3", &vvs_na3,
 "mscale_na3", &mscale_na3,
 "hscale_na3", &hscale_na3,
 "sscale_na3", &sscale_na3,
 0,0
};
...
double alpv ( _threadargsprotocomma_ double _lv , double _lvh ) {
   double _lalpv;
...
static void _hoc_alpv(void) {
  ...
  _nt = nrn_threads;
 _r =  alpv ( _p, _ppvar, _thread, _nt, *getarg(1) , *getarg(2) );
 hoc_retpushx(_r);
}
...
static int  trates ( _threadargsprotocomma_ double _lvm , double _la2 ) {
 ...
 _lqt = pow( q10 , ( ( celsius - 24.0 ) / 10.0 ) ) ;
   ...
   _lc = alpv ( _threadargscomma_ _lvm , vhalfs ) ;
...
static const char* nmodl_file_text = 
  ...
  "}\n"
  "\n"
  ":FUNCTION alpv(v(mV),vh(mV)) {    :used in \"s\" activation system infinity calculation, added by D Itkis 3/27/23, instead of sinf = c + a2*(1-c)\n"
